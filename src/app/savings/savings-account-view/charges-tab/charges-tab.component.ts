  ...
  payCharge(charge: any) {
    ...
    payChargeDialogRef.afterClosed().subscribe((response: any) => {
      if (response.data) {
        ...
        this.savingsService.executeSavingsAccountChargesCommand(this.savingsAccountData.id, 'paycharge', dataObject, charge.id)
          .subscribe(() => {
            this.reload();
          });
      }
